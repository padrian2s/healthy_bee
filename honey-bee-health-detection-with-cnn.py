...
# Global variables
img_folder='/Users/p3700676/api/bee_imgs/bee_imgs/'
img_width=100
img_height=100
img_channels=3
...
def main_ml():
    # # 2. Read Bee data
    # In[ ]:
    bees=pd.read_csv('/Users/p3700676/api/bee_data.csv',
                    index_col=False,
    ...
    img_exists = bees['file'].apply(lambda f: os.path.exists(os.path.join(img_folder, f)))
    bees = bees[img_exists]

    st.write(bees.head().astype('object'))
    ...
    for c in health_cats:
        ...
        ax[i].imshow(imageio.imread(os.path.join(img_folder, bee['file'])))
    ...
    # Train
    training2 = model2.fit_generator(generator.flow(train_X,train_y, batch_size=10)
                            ,epochs=20
                            ,validation_data=[val_X, val_y]
                            ,steps_per_epoch=50
